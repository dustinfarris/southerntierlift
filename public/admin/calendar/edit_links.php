...
function showLinks() {
	...
	$q = "SELECT link_id, state, city, company from ".$table_prefix."links where company != '' order by state, city, company";
	$query = mysql_query($q);
...
<table>
	...
	<tr>
		<td colspan="2">
			...
			<?php echo "<textarea cols=\"60\" rows=\"10\" name=\"description\" id=\"description\">".$cat["description"]."</textarea>\n";?>
		</td>
	</tr>
	...
	<?php if($cat['trucks']) {
		...
		foreach ($trucks as $key => $truck) {
			...
			<table class="trucks">
				<tr class="title">
					<td>Truck <?php echo ($c+1)?></td>
					<input id="array_id" type="hidden" value="<?php echo $c?>" name="array_id[]" />
				</tr>
				<tr>
					<td>Truck Make: </td>
	...
	<table class="trucks">
		<tr class="title">
			<td>Truck 1</td>
			<input id="array_id" type="hidden" value="0" name="array_id[]" />
		</tr>
		<tr>
			<td>Truck Make: </td>
...
<p id="addTruck">Add Truck</p>
<script type="text/javascript">
	$(document).ready( function() {
		$('#addTruck').click( function() {
			var c = parseInt($('tr.title:last #array_id').val()) +1;
			alert(c);
			var html = '<tr class="title"><td>Truck '+ c +'</td><input id="array_id" type="hidden" value="'+c+'" name="array_id[]" /></tr><tr><td>Truck Make: </td><td><input type="text" name="make[]" /></td></tr><tr><td>Truck Model: </td><td><input type="text" name="model[]" /></td></tr><tr><td>Truck Serial: </td><td><input type="text" name="serial[]" /></td></tr>';
			$('table.trucks').append(html);
		});
	});
</script>
</form>
