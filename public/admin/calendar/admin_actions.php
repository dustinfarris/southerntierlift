...
function addLink() {
	...
	if (!$edit_categories) {
		...
		// print_r($_POST);
		if(is_array($_POST['array_id'])) {
			//create json array
			$jsonArray = array();
			foreach($_POST['array_id'] as $row => $truck_id) {
					if($_POST['model'] == "" || $_POST['make'] == "" || $_POST['serial'] == "") {
						$msg = "Invalid Truck Information";
					} else {
						$jsonArray[$truck_id]['make'] = mysql_real_escape_string($_POST['make'][$row]);
						$jsonArray[$truck_id]['model'] = mysql_real_escape_string($_POST['model'][$row]);
						$jsonArray[$truck_id]['serial'] = mysql_real_escape_string($_POST['serial'][$row]);
					}
			}
			$jsonEncoded = mysql_real_escape_string(json_encode($jsonArray));


		}
		$query = mysql_query("INSERT INTO ".$table_prefix."links (company, address1, address2, city, state, zip, phone, fax, contact, email, url, description, trucks) VALUES ('".$company."', '".$address1."', '".$address2."', '".$city."', '".$state."', '".$zip."', '".$phone."', '".$fax."', '".$contact."', '".$email."', '".$url."', '".$description."', '".$jsonEncoded."')");
	...
	if (!$edit_categories) {
		...
		if(is_array($_POST['array_id'])) {
			...
			if(count($_POST['array_id']) > 1) {
				...
				$array_id = mysql_real_escape_string($_POST['array_id'][0]);
				$jsonArray[$array_id]['make'] = mysql_real_escape_string($_POST['make'][$array_id]);
				$jsonArray[$array_id]['model'] = mysql_real_escape_string($_POST['model'][$array_id]); 
				$jsonArray[$array_id]['serial'] = mysql_real_escape_string($_POST['serial'][$array_id]);
			}
				$jsonEncoded = json_encode($jsonArray);
		}
		$q = "UPDATE ".$table_prefix."links set company = '".$company."', address1 = '".$address1."', address2 = '".$address2."', city = '".$city."', state = '".$state."', zip = '".$zip."', phone = '".$phone."', fax = '".$fax."', contact = '".$contact."', email = '".$email."', url ='".$url."', description ='".$description."', trucks='".$jsonEncoded."' where link_id =".$id;
		$query = mysql_query($q);
...
function deleteLink($id) {
	...
	if (!$edit_categories) {
		...
		$msg .= "Dates: Deleted";
		$query = mysql_query("DELETE * FROM ".$table_prefix."events where contact_id =".$id);
		$msg .= "Events: Deleted";
